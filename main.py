...
def store_key(key, password_id):
    with open(passwords_dir + f"/key_{password_id}.bin", "wb") as file:
        file.write(key)

def load_key(password_id):
    with open(passwords_dir + f"/key_{password_id}.bin", "rb") as file:
        return file.read()
...
def store_password(password_id, password):
    create_passwords_dir()
    key = generate_key()
    store_key(key, password_id)
    encrypted_password = encrypt_data(password.encode(), key)
    with open(passwords_dir + f"/encrypted_password_{password_id}.txt", "wb") as file:
        file.write(encrypted_password)

def load_password(password_id):
    with open(passwords_dir + f"/encrypted_password_{password_id}.txt", "rb") as file:
        encrypted_password = file.read()
    key = load_key(password_id)
    return decrypt_data(encrypted_password, key).decode()
...
def passwd():
    ...
    store_password(password_id, password)
...
def main():
    ...
    print('''    [[blue]4[/blue]] - [yellow]Random password[/yellow]       [[blue]MY[/blue]] - [yellow]My password[/yellow]''')
    print()
    ...
    if a == "1":
        ...
        if os.path.exists(OneDrive):
            print("    [green]OneDrive[/green]: [blue]Installed[/blue]")
        else:
            print("    [green]OneDrive[/green]: [red]Not installed[/red]")
        print()
        ...
        if b == 1:
            ...
            SysMain = "Set-Service -Name 'SysMain' -StartupType Disabled"
            process = subprocess.Popen(["powershell", "-Command", SysMain], stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=True)
            stdout, stderr = process.communicate()

            seclogon = "Set-Service -Name 'seclogon' -StartupType Disabled"
            process = subprocess.Popen(["powershell", "-Command", seclogon], stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=True)
            stdout, stderr = process.communicate()

            dot3svc = "Set-Service -Name 'dot3svc' -StartupType Disabled"
            process = subprocess.Popen(["powershell", "-Command", dot3svc], stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=True)
            stdout, stderr = process.communicate()
            ...
            PcaSvc = "Set-Service -Name 'PcaSvc' -StartupType Disabled"
            process = subprocess.Popen(["powershell", "-Command", PcaSvc], stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=True)
            stdout, stderr = process.communicate()
        ...
        password_id = input("Enter password id: ")
        try:
            password = load_password(password_id)
